...
a <- NULL
b <- ETS(ETS(Trips ~ error("A") + trend("A") + season("A")))
c <- NULL
is_null(a)+is_null(b)+is_null(c)

list(ETS(Trips ~ error("A") + trend("A") + season("A")), ETS(Trips ~ error("A") + trend("A") + season("A")))[1]

l <- list(a, b ,c)
is_null(l[[1]])+is_null(l[[2]])+is_null(l[[3]])
...
forPipe <- function(data, h=12, report = F, names = NULL, models = NULL, nmodels=1) {
  nulls <- is_null(models[[1]])+is_null(models[[2]])+is_null(models[[3]])
  
  if(is_tsibble(data)==T && is_list(names)==T && is_list(models)==T) {
    if(nulls==3) 
      # no valid model
        fit <- NULL
      # one valid models
      ...
      if(nulls==2)  
          fit <- data %>% model(names[nmodels] = models[nmodels])
        # two valid models
      if(nulls==1)   
          fit <- data %>% model(names[nmodels] = models[nmodels], 
                                        names[nmodels-1] = models[nmodels-1])
        # three valid models
      if(nulls==0)  
          fit <- data %>% model(names[nmodels] = models[nmodels], 
                                        names[nmodels-1] = models[nmodels-1],
                                        names[nmodels-2] = models[nmodels-2])
      fc <- fit %>% forecast(h=h)
      if(report==T)
        report(fit)
...
forPipe <- function(data, h=12, report = F, models = NULL, nmodels=1) {
  nulls <- is_null(models[[1]])+is_null(models[[2]])+is_null(models[[3]])
  
  if(is_tsibble(data)==T && is_list(models)==T) {
    if(nulls==3) 
      # no valid model
        fit <- NULL
      # one valid models
      ...
      if(nulls==2)  
          fit <- data %>% model(m1=models[nmodels])
        # two valid models
      if(nulls==1)   
          fit <- data %>% model(m1=models[nmodels], 
                                m2=models[nmodels-1])
...
myNames <- list("ANN", "AAA", "MAM")
myModels <- list(ETS(Trips ~ error("A") + trend("N") + season("N")),
                 ETS(Trips ~ error("A") + trend("A") + season("A")),
                 ETS(Trips ~ error("M") + trend("A") + season("M")))

fp <- forPipe(aus_holidays, 12, T, myModels, nmodels = 3)
